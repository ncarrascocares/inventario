...
if ($total>=1 && $pagina<=$Npaginas) {
    ...
    foreach ($datos as $rows){
        ...
        <tr class="has-text-centered">
            ...
            <td>
                <a href="index.php?vista=update&user_id_update='.$rows['usuario_id'].'" class="button is-success is-rounded is-email">Actualizar</a>
            </td>
            <td>
                <a href="'.$url.$pagina.'&user_id_delete='.$rows['usuario_id'].'" class="button is-danger is-rounded is-email">Eliminar</a>
            </td>
        </tr> 
        ';
        $contador++;
...
if ($total>=1 && $pagina<=$Npaginas) {
    $tabla.='
        <p class="has-text-right">
            Mostrando usuarios <strong>'.$pag_inicio.'</strong> al <strong>'.$pag_final.'</strong> de un <strong>total de '.$total.'</strong>
        </p>';
}
$conexion=null;
echo $tabla;
